...
export default function AdminPortal({ onSignOut }) {
  ...
  // Smart Card & RFID Issuance state
  const [cardTab, setCardTab] = useState('spending');
        ...
        <main className="portal__content">
          ...
          {(activeNav === 'Card Reader & Smart Cards' || activeNav === 'Smart Card & RFID Issuance') && (
            <div className="animate-fade-up" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              <div className="page-header">
                <div>
                  <h1 className="page-header__title" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <CreditCard size={26} style={{ color: ADMIN_ACCENT }} /> Smart Card & RFID Reader Issuance Hub
                  </h1>
                  <p className="page-header__subtitle">
                    Admin-exclusive portal for encoding, issuing, re-encrypting, and managing physical RFID Student Spending Cards, Parent Pickup Passes, and Gate Verification.
                  </p>
                </div>
              </div>

              {cardNotice && (
                <div style={{ padding: '12px 18px', background: '#dcfce7', border: '1px solid #22c55e', color: '#14532d', borderRadius: 10, fontWeight: 700, fontSize: 13 }} className="animate-fade-up">
                  {cardNotice}
                </div>
              )}

              {/* Sub-navigation tabs */}
              <div style={{ display: 'flex', gap: 10, borderBottom: '2px solid var(--gray-200)', paddingBottom: 10, flexWrap: 'wrap' }}>
                <button
                  type="button"
                  onClick={() => setCardTab('spending')}
                  style={{
                    padding: '8px 16px',
                    borderRadius: 8,
                    border: 'none',
                    ...
                    fontSize: 13,
                    cursor: 'pointer',
                    background: cardTab === 'spending' ? ADMIN_BG : 'var(--gray-100)',
                    color: cardTab === 'spending' ? '#ffffff' : 'var(--gray-700)',
                  }}
                >
                  💳 Issue Student Spending Card
                </button>

                <button
                  type="button"
                  onClick={() => setCardTab('pickup')}
                  style={{
                    padding: '8px 16px',
                    borderRadius: 8,
                    border: 'none',
                    ...
                    fontSize: 13,
                    cursor: 'pointer',
                    background: cardTab === 'pickup' ? ADMIN_BG : 'var(--gray-100)',
                    color: cardTab === 'pickup' ? '#ffffff' : 'var(--gray-700)',
                  }}
                >
                  👨‍👩‍👧 Parent Pickup Card & Re-Encrypt
                </button>

                <button
                  type="button"
                  onClick={() => setCardTab('temp')}
                  style={{
                    padding: '8px 16px',
                    borderRadius: 8,
                    border: 'none',
                    ...
                    fontSize: 13,
                    cursor: 'pointer',
                    background: cardTab === 'temp' ? ADMIN_BG : 'var(--gray-100)',
                    color: cardTab === 'temp' ? '#ffffff' : 'var(--gray-700)',
                  }}
                >
                  🎫 Temporary Parent Pickup Card
                </button>

                <button
                  type="button"
                  onClick={() => setCardTab('verification')}
                  style={{
                    padding: '8px 16px',
                    borderRadius: 8,
                    border: 'none',
                    ...
                    fontSize: 13,
                    cursor: 'pointer',
                    background: cardTab === 'verification' ? ADMIN_BG : 'var(--gray-100)',
                    color: cardTab === 'verification' ? '#ffffff' : 'var(--gray-700)',
                  }}
                >
              ...
              {/* Form Card */}
              <div className="panel" style={{ background: '#ffffff', padding: 24, borderRadius: 14, border: '1px solid var(--gray-200)', boxShadow: '0 4px 20px rgba(0,0,0,0.04)' }}>
                {cardTab === 'spending' && (
                  <form onSubmit={(e) => {
                    e.preventDefault();
                    const s = students.find(st => st.id === cardForm.studentId || st.studentId === cardForm.studentId);
                    if (s && updateOnboardedStudent) {
                      updateOnboardedStudent(s.id, { rfidCardCode: cardForm.rfidChipUid || `CARD-${Date.now().toString().slice(-6)}` });
                    }
                    setCardNotice(`✅ Successfully encoded & issued Smart Spending Card (UID: ${cardForm.rfidChipUid || 'CARD-AUTO'}) to ${s?.fullName || 'Student'}! Daily Limit set to GHS ${cardForm.dailyLimit}.`);
                    setTimeout(() => setCardNotice(''), 8000);
                  }}>
                    <h3 style={{ fontSize: 16, fontWeight: 900, marginBottom: 16, color: ADMIN_BG }}>💳 Issue Student Services Spending Smart Card</h3>
                    
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                      <div>
                        <label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--gray-700)', marginBottom: 6 }}>Select Student</label>
                        <select
                          className="form-input"
                          value={cardForm.studentId}
                          onChange={(e) => {
                            const sId = e.target.value;
                            const s = students.find(st => st.id === sId || st.studentId === sId);
                            setCardForm({ ...cardForm, studentId: sId, rfidChipUid: s?.rfidCardCode || '' });
                          }}
                          required
                        >
                          <option value="">-- Choose Student --</option>
                          {students.map(s => (
                            <option key={s.id} value={s.id}>{s.fullName} ({s.studentId} - {s.level})</option>
                          ))}
                        </select>
                      </div>

                      <div>
                        <label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--gray-700)', marginBottom: 6 }}>Smart Card RFID Chip UID / Card Code</label>
                        <input
                          type="text"
                          className="form-input"
                          placeholder="e.g. 0009841234 (or tap USB Reader)..."
                          value={cardForm.rfidChipUid}
                          onChange={(e) => setCardForm({ ...cardForm, rfidChipUid: e.target.value })}
                          required
                        />
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
                      <div>
                        <label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--gray-700)', marginBottom: 6 }}>Daily Spending Limit (GHS)</label>
                        <input
                          type="number"
                          className="form-input"
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--gray-700)', marginBottom: 6 }}>Card Security Passcode / PIN</label>
                        <input
                          type="password"
                          className="form-input"
                          value={cardForm.securityPin}
                          onChange={(e) => setCardForm({ ...cardForm, securityPin: e.target.value })}
                        />
                      </div>
                    </div>

                    <button type="submit" className="workflow-button" style={{ background: ADMIN_BG, color: '#fff', padding: '10px 24px', borderRadius: 8, fontWeight: 900, cursor: 'pointer', border: 'none' }}>
                      <Save size={16} /> Encode & Issue Student Smart Card
                    </button>
                  </form>
                )}
                ...
                {cardTab === 'pickup' && (
                  <form onSubmit={(e) => {
                    e.preventDefault();
                    const s = students.find(st => st.id === cardForm.studentId || st.studentId === cardForm.studentId);
                    setCardNotice(`✅ Parent Pickup Card Issued & Re-Encrypted for ${s?.fullName || 'Student'} (Guardian: ${cardForm.parentName || s?.guardianName}). Ghana Card: ${cardForm.ghanaCard || 'Verified'}.`);
                    setTimeout(() => setCardNotice(''), 8000);
                  }}>
                    <h3 style={{ fontSize: 16, fontWeight: 900, marginBottom: 16, color: ADMIN_BG }}>👨‍👩‍👧 Issue Parent's Pickup Card & Re-Encrypt</h3>
                    
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                      <div>
                        <label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--gray-700)', marginBottom: 6 }}>Select Ward / Student</label>
                        <select
                          className="form-input"
                          value={cardForm.studentId}
                          onChange={(e) => {
                            const sId = e.target.value;
                            const s = students.find(st => st.id === sId || st.studentId === sId);
                            setCardForm({ ...cardForm, studentId: sId, parentName: s?.guardianName || '', parentPhone: s?.guardianPhone || '' });
                          }}
                          required
                        >
                          <option value="">-- Choose Student --</option>
                          {students.map(s => (
                            <option key={s.id} value={s.id}>{s.fullName} ({s.guardianName})</option>
                          ))}
                        </select>
                      </div>

                      <div>
                        <label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--gray-700)', marginBottom: 6 }}>Parent / Guardian Name</label>
                        <input
                          type="text"
                          className="form-input"
                          value={cardForm.parentName}
                          onChange={(e) => setCardForm({ ...cardForm, parentName: e.target.value })}
                          required
                        />
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
                      <div>
                        <label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--gray-700)', marginBottom: 6 }}>Delegate Ghana Card / National ID No.</label>
                        <input
                          type="text"
                          className="form-input"
                          placeholder="e.g. GHA-77291048-2"
                          value={cardForm.ghanaCard}
                          onChange={(e) => setCardForm({ ...cardForm, ghanaCard: e.target.value })}
                        />
                      </div>

                      <div>
                        <label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--gray-700)', marginBottom: 6 }}>Parent Pickup Card RFID Chip UID</label>
                        <input
                          type="text"
                          className="form-input"
                          placeholder="e.g. PICKUP-994182"
                          value={cardForm.rfidChipUid}
                          onChange={(e) => setCardForm({ ...cardForm, rfidChipUid: e.target.value })}
                          required
                        />
                    ...
                    <button type="submit" className="workflow-button" style={{ background: ADMIN_BG, color: '#fff', padding: '10px 24px', borderRadius: 8, fontWeight: 900, cursor: 'pointer', border: 'none' }}>
                      <ShieldCheck size={16} /> Encrypt & Issue Parent Pickup Card
                    </button>
                  </form>
                )}
                ...
                {cardTab === 'temp' && (
                  <form onSubmit={(e) => {
                    e.preventDefault();
                    setCardNotice(`✅ Temporary Parent Pickup Pass issued! Valid until ${cardForm.tempExpiry}. SMS authorization code sent to delegate phone.`);
                    setTimeout(() => setCardNotice(''), 8000);
                  }}>
                    <h3 style={{ fontSize: 16, fontWeight: 900, marginBottom: 16, color: ADMIN_BG }}>🎫 Issue Temporary Parent Pickup Pass</h3>
                    
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                      <div>
                        <label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--gray-700)', marginBottom: 6 }}>Target Student</label>
                        <select
                          className="form-input"
                          value={cardForm.studentId}
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--gray-700)', marginBottom: 6 }}>Temporary Pass Expiration Date</label>
                        <input
                          type="date"
                          className="form-input"
                          value={cardForm.tempExpiry}
                          onChange={(e) => setCardForm({ ...cardForm, tempExpiry: e.target.value })}
                          required
                        />
                      </div>
                    </div>
                    ...
                    <div style={{ marginBottom: 20 }}>
                      <label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--gray-700)', marginBottom: 6 }}>Delegate / Driver Notes & Authorization</label>
                      <textarea
                        rows="2"
                        className="form-input"
                        placeholder="Authorized relative or driver designated for pickup today only..."
                    ...
                    <button type="submit" className="workflow-button" style={{ background: ADMIN_BG, color: '#fff', padding: '10px 24px', borderRadius: 8, fontWeight: 900, cursor: 'pointer', border: 'none' }}>
                      <FileCheck size={16} /> Generate Temporary Pickup Pass
                    </button>
                  </form>
                )}

                {cardTab === 'verification' && (
                  <div>
                    <h3 style={{ fontSize: 16, fontWeight: 900, marginBottom: 16, color: ADMIN_BG }}>🛡 Gate Security Verification Terminal</h3>
                    <p style={{ fontSize: 13, color: 'var(--gray-600)', marginBottom: 20 }}>
                      Verify drop-off / pickup authenticity for morning drop-offs, afternoon pickup cards, or morning walker passes.
                    </p>

                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
                      <div>
                        <label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--gray-700)', marginBottom: 6 }}>Scan RFID Tag / Enter Student ID</label>
                        <input
                          type="text"
                          className="form-input"
                          placeholder="Tap card reader or type ID..."
                          value={cardForm.rfidChipUid}
                          onChange={(e) => setCardForm({ ...cardForm, rfidChipUid: e.target.value })}
                        />
                      </div>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--gray-700)', marginBottom: 6 }}>Verification Check Type</label>
                        <select className="form-input">
                          <option>Morning Drop-Off Verification</option>
                          <option>Afternoon Pickup Verification</option>
                          <option>Morning Walker Verification (Temp. Cards)</option>
                        </select>
                      </div>
                    </div>
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        setCardNotice(`🛡 GATE VERIFIED APPROVED! Card scan matched authorized guardian. Gate barrier opened.`);
                        setTimeout(() => setCardNotice(''), 8000);
                      }}
                      className="workflow-button"
                      style={{ background: '#059669', color: '#fff', padding: '10px 24px', borderRadius: 8, fontWeight: 900, cursor: 'pointer', border: 'none' }}
                    >
                      <UserCheck size={16} /> Authorize & Approve Verification
                    </button>
                  </div>
                )}
              </div>

              {/* Roster of Assigned Smart Cards */}
              <div className="panel">
                <h3 style={{ fontSize: 16, fontWeight: 900, marginBottom: 14 }}>Issued Smart Cards Directory</h3>
                <table className="data-table">
                  <thead>
                    <tr>
                      <th>Student Code</th>
                  ...
                  <tbody>
                    {students.map(s => (
                      <tr key={s.id}>
                        ...
                        <td><span className="level-badge">{s.level}</span></td>
                        <td>
                          <span style={{ fontFamily: 'monospace', fontWeight: 800, color: ADMIN_ACCENT }}>
                            💳 {s.rfidCardCode || 'CARD-UNASSIGNED'}
                          </span>
                        </td>
                        ...
                        <td>
                          <button
                            ...
                            onClick={() => {
                              setCardTab('spending');
                              setCardForm({ ...cardForm, studentId: s.id, rfidChipUid: s.rfidCardCode || '' });
                            }}
                            style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid var(--gray-300)', background: '#fff', fontSize: 11, fontWeight: 800, cursor: 'pointer' }}
                          >
                            Re-Issue / Edit
                          </button>
                        </td>
